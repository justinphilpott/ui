...
### Update astro.config.mjs to define an import alias to the src directory.

To allow import aliases such as "@/components/..." to properly resolve, add the following vite alias to astro.config.mjs:

```ts {8-14} showLineNumbers
export default defineConfig({
  integrations: [
    react(), 
    tailwind({
